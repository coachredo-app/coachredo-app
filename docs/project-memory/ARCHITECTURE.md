...
**Écritures d'autosauvegarde du Bilan — via Server Actions depuis le 2026-09-11 (D-017), plus de client Supabase navigateur.** `bilan_responses` (upsert/delete des réponses) et `bilan_sessions.current_step` (navigation d'étape) étaient jusqu'ici écrits directement depuis le navigateur (`src/lib/reader/bilan-sync.ts`, `createBrowserClient`), gouvernés uniquement par RLS. Ils passent désormais par deux Server Actions dédiées (`saveBilanResponseAction`, `updateCurrentStepAction`, `src/app/(app)/bilan/actions.ts`), qui utilisent le client Supabase **authentifié** de l'utilisateur (jamais `service_role`) — l'ownership reste garanti par les mêmes policies RLS qu'avant (`bilan_responses_insert/update/delete_active_session`, `bilan_sessions_update_step_own`, migration `005_bilan_sessions.sql`), **inchangées**. Ce changement a été introduit pour permettre à ces écritures de vérifier un flag serveur (`BILAN_OPEN`, voir ci-dessous) avant d'écrire — chose impossible tant qu'elles partaient directement du navigateur. `bilan-sync.ts` conserve exactement les mêmes signatures publiques ; `BilanReader.tsx` n'a pas eu besoin d'être modifié.

**Mécanisme de fermeture temporaire — flag `BILAN_OPEN` (`src/lib/bilan-flag.ts`).** Premier (et à ce jour seul) mécanisme de feature flag du repo — une simple variable d'environnement (`process.env.BILAN_OPEN !== 'false'`), sans librairie ni architecture générique. Portée actuelle strictement limitée au Bilan de clarté : guard dans `bilan/page.tsx` (distingue aucune session / `in_progress` / `completed` V2 / legacy V1 sans V2), neutralisation de `/bilan/upgrade`, et check en tête de chaque Server Action d'écriture de `bilan/actions.ts`. **Pattern à réutiliser tel quel** si un futur besoin de fermeture temporaire similaire apparaît ailleurs dans le repo (flag env + guard côté Server Component/Server Action, jamais évalué côté client seul) — plutôt que d'introduire une nouvelle architecture de feature flags. Détail complet : DECISIONS D-017.
