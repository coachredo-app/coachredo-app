...
export default async function BilanPage() {
  ...
  // superseded seul ≠ Bilan validé — seul un completed prouve un historique V1 réel
  ...
  // Cas E1 : session legacy in_progress sans historique V1 completed
  //   → migration transparente vers V2 standard (pas un upgrade)
  if (isLegacyStandardActive && !hasV1History) {
    const { data: migrateResult } = await supabase.rpc('migrate_legacy_session')
    if (migrateResult?.error || !migrateResult?.session_id) redirect('/fr/dashboard')

    const { data: v2Session } = await supabase
      .from('bilan_sessions')
      .select('id, session_num, statut, current_step, started_at, completed_at, bilan_version, session_type')
      .eq('id', migrateResult.session_id)
      .single()
    if (!v2Session) redirect('/fr/dashboard')

    const { data: v2Rows } = await supabase
      .from('bilan_responses')
      .select('question_id, response, updated_at')
      .eq('session_id', v2Session.id)

    const e1Responses = Object.fromEntries(
      (v2Rows ?? []).map(r => [r.question_id as string, r.response as string])
    )

    return (
      <BilanReader
        session={v2Session}
        initialResponses={e1Responses}
        upgradeMode={false}
  ...
  // Cas A : legacy standard in_progress + historique V1 completed → ne pas servir cette session,
  //          laisser Cas B détecter le legacy et rediriger vers /bilan/upgrade.
  // Cas normal : session active V2 in_progress → servie directement.
